...
if (!class_exists('WC_SA_Order')) :
    ...
    class WC_SA_Order
    {
        ...
        public function get_trigger_time($trigger_time)
        {
            ...
            if (!$trigger_options['weekday']) {
                $trigger_time = 0;
            }

            if ($trigger_options['weekday'] && !$trigger_options['weekday'][$trigger_day]) {
        ...
        public function change_order_labels($labels){
            $custom_statuses = get_option('wc_custom_status_edit_existing_status');
            foreach ($custom_statuses as $key => $status){
                $labels[$key] = preg_replace('/>(\w+) </', '>' . $status['name'] . ' <', $labels[$key]);
            }
